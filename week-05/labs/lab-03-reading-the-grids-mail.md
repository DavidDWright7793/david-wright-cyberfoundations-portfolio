...
```
The Source column tells you which machine the packet came from.
```

What the Destination column tells you:

```
The Destination column tells you which machine the packet was sent to.
```

What the Protocol column tells you:

```
The Protocol column tells you what protocol was used to send the packet.
```

What the Info column tells you:

```
The Info column tells you a summary of what happened in the packet.
```
...
```
The workstation address is 10.20.5.42, as it is the address sending most of the requests.
```
...
```
The packet inspector showed me the actual network packets being transmitted and received, whereas dig only asks a DNS server which IP address is mapped to a hostname. A packet inspector shows much more about what actually happened on the nwtwork.
```
...
```
The two port numbers are 51514 and 443. The port number that belongs to the server is 443.
```
...
```
I would be able to see a packet that contains data that looks like it was only intended for storeroom staff.
```
...
```
Packet 10's detail pane shows that it was sent using the TLS protocol, and therefore the data in the packet is encrypted.
```
...
```
It seems that Packet 10 contains information that should only be visible to certain parties, like Packet 14. The difference is that Packet 10 is encrypted, while Packet 14 is not.
```
...
```
In a packet inspector, a filter is a view that narrows the packets displayed for examination based on your desired criteria. It does not delete or remove the other packets from the capture-it merely hides the packets that don't match the filter. One situation where confusing a view of filtered files with the absence of other files could lead an analyst to a wrong conclusion would be if the analyst is troubleshooting DNS and filters for traffic involving the destination host. If they see no packets in the filter, they could mistakenly conclude that the machine never communicated, but the DNS and request and response might have been captured-they are just outside the current filter. Therefore, they could miss evidence that DNS worked and reach an incorrect conclusion about where the failure occurred. In a nutshell, an analyst might mistakenly assume that packets outside of the filter do not exist, leading them to a potentially draw conclusions from an artifically narrow view of the capture.
```
...
```
The dig command is useful when you need to quickly verify whether a particular DNS query successfully receives a response and which IP address the hostname resolves to. It directly gives you information about which DNS server answered the query, the query status, and how long the query took. The packet view in a packet inspector provides much more detailed information about the underlying transaction and is useful for investigating how the DNS exchange occurred and diagnosing more detailed DNS or network issues. An example of an investigation where I would specifically want the packet view is a situation where sometimes a certain hostname resolves, and sometimes it doesn't. A packet inspector can provide detailed information about the intermittent behavior of a DNS resolution, not merely whether the name resolved or not.
```
...
```
The knock-at-the-door analogy is very insightful for explaining how a TCP handshake works. The querying port knocks on the answering port (SYN), the responding port replies "Who's there? Come in." (SYN-ACK), and then the querying port then says "Thanks" and enters the door (ACK). This three-way handshake establishes the TCP connection and synchronizes the sequence numbers that TCP uses to ensure reliable, ordered delivery of data.
```
...
```
I think the reason I could read packet 14 verbatim but not read packet 10 at all is that packet 10 contains TLS-encrypted traffic, while packet 14 contains HTTP traffic that was transmitted without TLS encryption. The fact that packet 14 contains a staff code and is unencrypted is significant because the staff code is readable by anyone who can observe that network traffic. An attacker who captures code on that network can read the code directly, which potentially exposes information to individuals who are not authorized to view it.  The issue is not that the staff code appears in a packet, but that the code is being transmitted in plaintext, making it potentially visible to anyone who can capture the network traffic.
```
