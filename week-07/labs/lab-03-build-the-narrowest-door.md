...
```text
1. If no student Allow exists, Grid Beacon should receive a Deny verdict.
2. After the narrow Allow is created and the service is listening, Grid Beacon should receive an Allow verdict.
3. The first case is decided by the protected priority 1000 fallback Deny, as there is no student Allow. The second case is decid3ed by the student Allow, as the Allow would be a priority between 200-999, which would take precedence over the fallback Deny at priority 1000.
```
...
```text
Name: allow-grid-beacon-8080, Priority: 300, Direction: Inbound, Action: Allow, Protocol: TCP, Source: 10.6.6.4, Source Port: *, Destination: *, Destination port: 8080, Description: Week 7 least-privilege test service
```
...
```text
Allowing one source to one TCP port is narrower than allowing the whole subnet or Any source for one simple reason. Even if all three rules might make the intended test pass, allowing only one source is the safest option, as it only allows one sender to access one port. If you allow the whole subnet, many sources can access the TCP port, not only the intended source. If you allow Any source, everything is able to reach you. So all three rules get the job (letting in the correct source) done, but the first rule is more secure than the other two because it allows only the source who needs access to enter and no others. The other two rules permit traffic from the designated source, but they permit traffic from many other sources as well.
```
...
```text
The Python service controls what an application does once a connection reaches it. It controls things such as what port it listens on, what requests it accepts, and what responses it sends. The NSG rule controls whether network traffic is allowed to reach the VM/subnet in the first place.
```

**Analysis Question 2.** Why is `10.60.6.4` a stronger source value than Any for this task? (Minimum 4 sentences.)

```text
The reason that '10.60.6.4' is a stronger source value than Any for this task is simply that '10.60.6.4' is a narrower value. Putting '10.60.6.4' as the source value only allows entry to that one source, whereas a value of Any allows any source entry. Both values allow the intended source to get through, but a value of Any allows everyone else to get through as well. Rules should always be kept as narrow as possible in order to allow only the required traffic through, and no more.
```

**Analysis Question 3.** What does an `ALLOWED` result prove, and what does it not prove about other sources? (Minimum 3 sentences.)

```text
An 'Allowed' result proves that a specific source is allowed access. It does not prove that any other source is allowed or denied access. Each source is evaluated independently, so the result of one traffic flow cannot determine the result of a different traffic flow.
```
