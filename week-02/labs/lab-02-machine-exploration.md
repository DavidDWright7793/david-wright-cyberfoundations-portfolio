...
```
My computer has 32.0 GB of RAM installed.
```
...
```
My computer has 954 GB of storage capacity.
```

**Free storage:** 783 GB

```
My computer currently has 783 GB of storage capacity available.
```
...
```
"Windows 11 Home, Version 25H2" describes the operating system. In contrast, 32.0 GB describes the RAM and 954 GB describes the storage, which are both components of the hardware.
```
...
```
Google Chrome is one process that my computer is currently running. It is an example of software, as it is an application (web browser/search engine) that the user (me) utilizes to interact with the computer. Google Chrome then relays my request to the OS, which then passes it to the hardware. The hardware then passed the requested information to the OS, which then relays it to the application (software), which then displays the information to the user (me).
```
...
```
It is important for a cybersecurity professional to monitor the CPU usage number on a computer, because an abnormally large increase or decrease in CPU usage could indicate that there is a malicious process running on the computer that is either excessively using CPU resources, or preventing them from being used. It could also mean that malware has unknowingly gotten onto the computer if there is a process that is using resources that were previously not used. In other words, unexpected changes should be monitored, as they might not be good changes.
```
...
```
Watching my machine live showed me the changes in CPU usage moment to moment, which gave me an idea of how my computer is actually running. A static diagram cannot give a user that kind of complete picture, as it only records one moment.
```
