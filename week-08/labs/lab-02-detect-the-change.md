...
```text
If one line is added to a file, its SHA-256 digest will not remain the same. Every input produces a unique fixed-length digest. If even one character in a file is changed, its digest will be completely different.
```
...
```text
The mismatch in hashes proves that the original file was changed. Even the tiniest change to the file would produce a completely different digest than the file had previously. However, a mismatch of hashes does not prove who altered the file. It also does not prove what in the file was altered-it only proves that the file was altered.
```
...
```text
A different SHA-256 value proves that the file has been altered, as even a change to one character in a file produces a completely different digest.
```

2. Why does the mismatch not identify the person who changed the file?

```text
A mismatch in hashes only proves that a file has been changed. It does not identify who changed a file-that is not the objective of hashing. A digest identifies a data state at a moment in time, not the author or creator of the data.
```

3. Why does hashing not protect confidentiality?

```text
Hashing does not protect confidentiality because hashing is designed to protect the integrity of data, not the confidentiality of data. A digest is not data than can be encrypted or decrypted-it is a mathematical fingerprint of data, and does not tell you what the data actually is.
```
