...
**[TERMINAL] Run:** whoami, hostname, cf-week8-check, cd ~/cloud-heights/week8-cryptography, pwd
...
```text
This lab demonstrates confidentiality because it encrypts a file, and then decrypts it with the correct key. Encrypting a file and only allowing it to be decrypted with the correct key helps ensure that it can only be viewed by authorized parties, thus preserving the pillar of confidentiality. Encryption does not prevent deletion of a file because encryption only addresses the pillar of confidentiality, i.e. keeping the information readable only by those who are supposed to read it. This is a separate issue from deleting a file, as an encrypted file can still be accidentally or intentionally deleted-encryption only prevents unauthorized parties from reading the file, not from deleting it.
```
...
```text
An encrypted file can still be copied or deleted because encryption can only protects the confidentiality of data, not the availability or integrity of data. It only prevents an unauthorized user from reading the date, not from copying or stealing it. Encryption is not a complete defense on its own.
```

2. What job did the passphrase perform?

```text
The passphrase was used to encrypt and decrypt the file.
```

3. Why does the match test prove correct recovery but not prove who handled the file?

```text
THe match test proces correct recovery because that is the actual job of encryption-making data unreadable to everyone except the parties authorized to view it. It does not prove who handled the file because that is not the job of encryption-encryption cannot tell you who accessed or handled a file, only prevent those who are not authorized to read it from doing so.
```
