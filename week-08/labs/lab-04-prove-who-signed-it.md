...
```text
The original signature should not verify after the report content changes. If the content changes by even one character after signing, verification fails-that is the integrity guarantee.
```
...
```text
The first verification result ('Verified OK') occurred because at the time, the file had not been changed. I then changed the file during the lab, and when I tried to verify again, the result was 'Verification failure'. The reason the second verification failed is because I made a change to the report during the course of the lab, and a change to even one character in a file causes the verification to fail. Signing is not encryption because a digital signature produces a signature value that can be verified-it does not encrypt the document. The document remains readable with signing alone-signing and encryption are separate operations serving different security goals.
```
...
```text
In the lab, the result of 'Verified OK' established that the document had not been altered since it was digitally signed, and the signature remained valid.
```

2. Why did the changed copy fail verification?

```text
The changed copy failed verification because I had altered the file. If even one character in a file is changed after it is digitally signed, the verification will fail.
```

3. Why is the signed report still readable?

```text
A signed report is still readable because a digital signature does not encrypt files-it produces a signature value that can be verified. A digital signature safeguards the integrity of a file, not the confidentiality. Signing and encryption do two separate jobs that serve different security goals.
```

4. Why does the signature alone not prove which human used the private key?

```text
A valid signature proves that the corresponding private key was used and the content is unchanged. It does not guarantee that a specific human pesonally acted-that is the function of key custody.
```
