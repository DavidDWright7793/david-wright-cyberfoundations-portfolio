...
**[TERMINAL] Run:** whoami, cf-wek8-check, test ! -e ~/.ssh/week8_analyst_ed25519 &&   test ! -e ~/.ssh/week8_analyst_ed25519.pub &&   echo "READY: Week 8 key path is unused" ||   echo "STOP: Week 8 key already exists"
...
```text
The public key can be distributed when appropriate because it is what others need to communicate securely with the owner, and is meant to be shared. A public key is like the address of your house-it enables others to find you in order to communicate with you. A private key is like the key to your house-only you can actually get inside your house, and you do not free distribute your house key.
```
...
**[TERMINAL] Run:** ssh-keygen -lf ~/.ssh/week8_analyst_ed25519.pub
...
```text
A public key is designed to be shared with others and is placed in the authorized_keys file on a server. A private key is designed to only be known to the owner, and thus is never sent to a server-it stays on the client machine. The key passphrase protects the private key file sitting on the owner's machine-it is not the same as an account password.
```
...
```text
A public key can be distributed because it is used for others to securely communicate with the owner of a file-it is used to encrypt and verify, and is designed to leave the owner's machine. A private key must remain protected because that is what the owner uses to securely access his own file and allow others access to the file-it is used to decrypt and sign, and never leaves the owner's machine.
```

2. How is the key passphrase different from the `analyst` account password?

```text
A key passphrase is different from the 'analyst' account password because they do fundamentally different jobs. A key passphrase protects the private key file on disk, while the account password unlocks the account.
```

3. Why did you use a unique Week 8 filename?

```text
I used a unique Week 8 filename because I needed to create a new file with a public key, private key, and passphrase. I needed to create a new file from scratch in order to add the access controls one by one.
```
