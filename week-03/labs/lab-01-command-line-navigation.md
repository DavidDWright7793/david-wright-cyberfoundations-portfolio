...
```
The grep command searches for your specified pattern in each file.
```
...
```
The path '/home/morgan' that was put out when I entered the command 'pwd' is written in Linux style. One clue that it is Linux, and not Windows, is that the path includes forward slashes (/), not backslashes (\) like in Windows. Another indicator is the fact that drive letters are absent from the path, as they are used in Windows paths, but not Linux paths.
```
...
```
The "move, then check" habit matter for several reasons. One reason is that you want to confirm where you have navigated to, so you are where you wanted to go. Another reason is that you want to confirm where you are before you make any changes, so you can make the changes you want and avoid making changes that you don't want.
```
...
```
The habit of asking the terminal for help instead of memorizing every command in advance is advantageous for IT and cybersecurity professionals for several reasons. One reason is to confirm the correct command before you perform the action yu want to perform, so you actually use the command that achieves your desired function. Another reason is that there are a huge number of commands-it is practically impossible to memorize all of them and then automatically know the correct command to use in any given situation off the top of your head.
```
...
```
I have experience navigating file systems, as I took a coding bootcamp in which I used PowerShell extensively. I would say that I did not find the file-system tree for this lab that hard to navigate-it seemed that everything was easy to get to and in clearly designated places.
```
