...
| 2. Look around | ls | ls |
| 3. Move into a folder | cd | cd  |
| 4. Peek inside a file | cat  | cat |
...
```
One major difference I noticed between the way the two shells presented information was the column layout. Bash listed all in one line, while PowerShell listed the files in columns, with one file on each line.
```
...
```
There are a few indicators that the underlying file system tree was identical across both Bash and PowerShell. One clue is that the folder that contained the file I examined was titled 'incident-42' in both passes, and the file I examined was titled 'access-log.txt' in both passes as well. Additionally, the contents of 'access-log.txt' in both Bash and PowerShell were identical.
```
...
```
I would say that pwd vs. Get-Location pair felt the most different to me. I was much more comfortable using pwd than Get-Location, because the former is much shorter and simpler than the latter. The former is 3 simple letters, while the latter is two words with a hyphen separating them, and both words need to be capitalized.
```
...
```
A significant difference between Bash and PowerShell that was visible in my output during this lab was the structure of the contents of folders. In Bash, the folder and files names are all listed vertically side-by-side on one line. In PowerShell, they are listed individually on one row each, in columns.
```
